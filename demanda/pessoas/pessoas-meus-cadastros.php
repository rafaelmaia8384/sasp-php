<?php
	...
	if (!empty($_POST['index'])) {
        ...
        $result = $this->db->dbRead("SELECT * FROM tb_pessoas WHERE cpf_usuario = {$cpf} AND pessoa_excluida = 0 ORDER BY data_registro DESC LIMIT {$limit}, {$search_limit}");
        ...
        if (is_array($result)) {
            ...
            $this->db->saspError('Você não cadastrou nenhuma pessoa.');
        }
	}
	else {
