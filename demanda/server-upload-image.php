<?php
    ...
    if (!empty($_POST['modulo'])) {
        ...
        if ($modulo === 'pessoas') {
            ...
            $result3 = $this->db->dbRead("SELECT id FROM tb_pessoas_marca_corporal WHERE img_busca = '{$imgBuscaName}' AND img_principal = '{$imgPrincipalName}' LIMIT 1");
        ...
        if ($modulo === 'pessoas') {
            ...
            $this->db->dbExecute("UPDATE tb_pessoas_marca_corporal SET img_enviada = 1 WHERE img_busca = '{$imgBuscaName}' AND img_principal = '{$imgPrincipalName}' LIMIT 1");
        }
